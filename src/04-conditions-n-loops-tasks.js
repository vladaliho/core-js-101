...
function doRectanglesOverlap(rect1, rect2) {
  ...
  // Проверяем на отсутствие наложения
  const noOverlap1 = rect1Right < rect2.left; // rect1 справа от rect2
  const noOverlap2 = rect2Right < rect1.left; // rect2 справа от rect1
  const noOverlap3 = rect1Bottom < rect2.top; // rect1 ниже rect2
  const noOverlap4 = rect2Bottom < rect1.top; // rect2 ниже rect1

  return !(noOverlap1 || noOverlap2 || noOverlap3 || noOverlap4);
}
...
function getMatrixProduct(m1, m2) {
  ...
  const product = Array.from({ length: rows1 }, () => Array(cols2).fill(0));
...
function evaluateTicTacToePosition(position) {
  ...
  winningCombinations.forEach((combination) => {
    ...
    const first = position[a[0]][a[1]];
    const second = position[b[0]][b[1]];
    const third = position[c[0]][c[1]];
